...
def main():
    ...
    print()
    print(
        f"Direct Gamma(k, 1/λ):       mean = {direct.mean().item():.4f}, "
        f"var = {direct.var().item():.4f}"
    )
    print(
        f"Sum of k Exponential(λ):    mean = {summed.mean().item():.4f}, "
    ...
    for lvl, qd, qs in zip(quantile_levels.tolist(), q_direct, q_summed, strict=True):
